...
/* ══════════════════════════════════════════
   SOVEREIGN — OBSIDIAN & BURNISHED GOLD
...
@keyframes orb-b {
    ...
    65%{transform:translate(50px,-60px) scale(0.95);}
}
...
.signal-card {
    ...
    background:linear-gradient(135deg,#1a1620 0%,#100e14 100%);
...
.signal-card::after {
    ...
    background:radial-gradient(ellipse at 30% 50%,rgba(201,168,76,0.04),transparent 60%);
    pointer-events:none;
}
.signal-badge {
    font-family:'Space Mono',monospace; font-size:0.52rem;
...
.signal-headline {
    ...
    color:var(--ivory); line-height:1.4; margin-bottom:0.4rem;
}
.signal-sub {
...
.alert-strip {
    ...
    font-family:'Space Mono',monospace; font-size:0.62rem; letter-spacing:1px;
...
.alert-strip.critical {
    background:rgba(181,42,42,0.06); border-color:rgba(181,42,42,0.25);
    border-left-color:var(--red); color:var(--red);
}
.alert-strip.clear {
    background:rgba(45,122,79,0.06); border-color:rgba(45,122,79,0.2);
    border-left-color:var(--green); color:#4aad74;
}

/* ── SECTION LABEL ── */
.s-label {
    font-family:'Space Mono',monospace; font-size:0.56rem; letter-spacing:5px;
...
.stat-block {
    ...
    border-top:2px solid var(--border2); padding:1.2rem;
    transition:all 0.3s ease; cursor:default;
}
.stat-block:hover {
    border-top-color:var(--gold);
    box-shadow:0 0 30px rgba(201,168,76,0.08),inset 0 0 20px rgba(201,168,76,0.03);
}
.stat-val {
...
.v-card-a { border-left:2px solid var(--gold); background:rgba(201,168,76,0.03); }
.v-card-g { border-left:2px solid #2d7a4f; background:rgba(45,122,79,0.04); border-color:rgba(45,122,79,0.15); }
.v-card-r { border-left:2px solid var(--red); background:rgba(181,42,42,0.05); border-color:rgba(181,42,42,0.15); }
.v-card-b { border-left:2px solid #5b8fc9; background:rgba(91,143,201,0.04); border-color:rgba(91,143,201,0.15); }
.v-card-p { border-left:2px solid #a07ee0; background:rgba(160,126,224,0.04); border-color:rgba(160,126,224,0.15); }
.v-card-a:hover { box-shadow:0 0 18px rgba(201,168,76,0.1); border-left-width:3px; }
.v-card-g:hover { box-shadow:0 0 18px rgba(45,122,79,0.1); }
.v-card-r:hover { box-shadow:0 0 18px rgba(181,42,42,0.1); }
.v-card-b:hover { box-shadow:0 0 18px rgba(91,143,201,0.1); }
.news-card { transition:all 0.2s ease; }
.news-card:hover { border-color:var(--gold)!important; transform:translateX(4px); box-shadow:0 0 14px rgba(201,168,76,0.08); }
...
/* ── PROGRESS ── */
.v-bar-wrap { background:var(--dim2); height:3px; border-radius:0; margin:0.5rem 0; }
.v-bar-fill {
    height:3px; background:linear-gradient(90deg,var(--gold-lo),var(--gold),var(--gold-hi));
...
.v-bar-fill.b{background:linear-gradient(90deg,#2a4f80,#5b8fc9,#85b4e8);box-shadow:0 0 8px rgba(91,143,201,0.4);}
...
.vpill-a{background:rgba(201,168,76,0.1);color:var(--gold);border:1px solid rgba(201,168,76,0.3);}
.vpill-g{background:rgba(45,122,79,0.1);color:#4aad74;border:1px solid rgba(45,122,79,0.3);}
.vpill-r{background:rgba(181,42,42,0.1);color:var(--red);border:1px solid rgba(181,42,42,0.3);}
.vpill-b{background:rgba(91,143,201,0.1);color:#5b8fc9;border:1px solid rgba(91,143,201,0.3);}
.vpill-p{background:rgba(160,126,224,0.1);color:#a07ee0;border:1px solid rgba(160,126,224,0.3);}
...
/* ── THOUGHTS ── */
.thought-block {
    background:var(--bg2); border:1px solid var(--border); border-top:2px solid var(--violet);
    padding:1rem 1.2rem; margin-bottom:0.5rem;
    font-size:0.85rem; color:var(--text); line-height:1.7;
    transition:all 0.25s ease;
}
.thought-block:hover { border-top-color:var(--gold); box-shadow:0 0 16px rgba(201,168,76,0.06); }
.thought-stamp { font-family:'Space Mono',monospace; font-size:0.56rem; color:var(--dim); margin-top:0.4rem; letter-spacing:1px; }

/* ── SYNAPSE ── */
.syn-id { font-family:'Space Mono',monospace; font-size:0.6rem; color:#4aad74; word-break:break-all; line-height:1.8; letter-spacing:1px; }
...
.news-stamp { font-family:'Space Mono',monospace; font-size:0.56rem; color:var(--dim); letter-spacing:1px; margin-top:0.3rem; }
...
.stSelectbox>div>div { background:var(--bg2)!important; border:1px solid var(--border2)!important; border-radius:0!important; }
.stButton>button {
    ...
    letter-spacing:3px!important; font-weight:700!important; text-transform:uppercase!important;
    transition:all 0.2s ease!important;
}
.stButton>button:hover {
    background:var(--gold)!important; color:#0d0c10!important;
    box-shadow:0 0 25px rgba(201,168,76,0.4)!important;
}
[data-testid="stMetric"] {
    background:var(--bg2)!important; border:1px solid var(--border)!important;
    border-top:2px solid var(--border2)!important; border-radius:0!important; padding:1rem!important;
    transition:all 0.3s ease!important;
}
[data-testid="stMetric"]:hover { border-top-color:var(--gold)!important; }
[data-testid="stMetricValue"] { color:var(--gold)!important; font-family:'Space Mono',monospace!important; }
[data-testid="stMetricLabel"] { color:var(--dim)!important; font-family:'Space Mono',monospace!important; font-size:0.6rem!important; letter-spacing:2px!important; }
.stTabs [data-baseweb="tab"] { font-family:'Space Mono',monospace!important; font-size:0.62rem!important; letter-spacing:3px!important; color:var(--dim)!important; }
.stTabs [aria-selected="true"] { color:var(--gold)!important; }
.stTabs [data-baseweb="tab-highlight"] { background:var(--gold)!important; }
.stProgress>div>div { background:linear-gradient(90deg,var(--gold-lo),var(--gold))!important; }
div[data-baseweb="select"] * { color:var(--text)!important; background:var(--bg2)!important; }
label { color:var(--dim)!important; font-family:'Space Mono',monospace!important; font-size:0.72rem!important; letter-spacing:1px!important; }

/* ── SCROLLBAR ── */
::-webkit-scrollbar { width:3px; }
::-webkit-scrollbar-track { background:var(--bg); }
::-webkit-scrollbar-thumb { background:linear-gradient(180deg,var(--gold-lo),var(--gold)); box-shadow:0 0 6px rgba(201,168,76,0.4); }

/* ── SELECTION ── */
::selection { background:rgba(201,168,76,0.2); color:var(--gold); }

/* ── FOOTER ── */
.vos-footer {
    text-align:center; padding:2rem;
    font-family:'Space Mono',monospace; font-size:0.52rem; letter-spacing:4px;
...
def generate_signal():
    ...
    # Market anomalies
    for item, logs in {**tt_nd, **tt_massy}.items():
        if len(logs) >= 2:
            curr,prev = logs[-1]["price"],logs[-2]["price"]
            if prev > 0:
                pct = (curr - prev) / prev * 100
                if abs(pct) > 12:
                    direction = "spiked" if pct > 0 else "dropped"
                    signals.append({
                        "icon":"📊","type":"MARKET ANOMALY",
                        "headline": f"{item.capitalize()} prices {direction} {abs(pct):.0f}% recently.",
                        "sub": f"Moved from TT${prev:.2f} to TT${curr:.2f}. Worth adjusting your shopping.",
                        "color":"var(--red)" if pct > 0 else "#4aad74"
                    })

    # Stalled goals
    stalled = [g for g in goals_data if g['status']=='Active' and g.get('progress',0) < 15]
    if stalled:
        g = random.choice(stalled)
        signals.append({
            "icon":"🎯","type":"GOAL SIGNAL",
            "headline": f"'{g['name']}' hasn't moved.",
            "sub": f"Progress is at {g.get('progress',0)}%. What's the one small thing that would unstick it?",
            "color":"var(--gold)"
        })

    # Mood pattern
    if len(thoughts_data) >= 4:
        mood_map = {"😊 Positive":8,"😐 Neutral":5,"😔 Low":2,"😤 Frustrated":3,"🤩 Energized":9,"😰 Anxious":3,"😌 Calm":7}
        recent = [mood_map.get(t.get('mood_tag','😐 Neutral'),5) for t in thoughts_data[-5:]]
        avg = sum(recent)/len(recent)
        if avg < 4:
            signals.append({
                "icon":"🧠","type":"PATTERN DETECTED",
                "headline": "Your last few log entries have been heavy.",
                "sub": "The data sees it even when you don't say it directly. What's actually going on?",
                "color":"var(--violet)"
            })
        elif avg > 7.5:
            signals.append({
                "icon":"⚡","type":"ENERGY SIGNAL",
                "headline": "Your mood data shows you're running hot right now.",
                "sub": "High energy states are rare. This is the window to tackle the thing you've been avoiding.",
                "color":"#4aad74"
            })

    # Unresolved decision
    pending_d = [d for d in decisions_data if d.get('recommendation') in ['THINK MORE','PROCEED WITH CAUTION']]
    if pending_d:
        d = random.choice(pending_d)
        signals.append({
            "icon":"⚖️","type":"UNRESOLVED",
            "headline": f"You still haven't fully decided: '{d['title'][:55]}'",
            "sub": "Unresolved decisions sit in the background draining energy. Even a provisional decision beats none.",
            "color":"var(--gold)"
        })

    # Hydration
    if h['water'] == 0 and now.hour > 10:
        signals.append({
            "icon":"💧","type":"BODY SIGNAL",
            "headline": "You haven't logged a single glass of water today.",
            "sub": "Dehydration degrades decision-making before you notice it. Your brain is ~75% water.",
            "color":"#5b8fc9"
        })

    # Sleep debt
    if h['sleep_hrs'] < 6 and h['sleep_hrs'] > 0:
        signals.append({
            "icon":"😴","type":"RECOVERY DEFICIT",
            "headline": f"You logged {h['sleep_hrs']}h sleep. That's a debt, not rest.",
            "sub": "Sleep deprivation compounds. What you lose in recovery costs more tomorrow.",
            "color":"var(--red)"
        })

    # Provocative statements (always available as fallback)
    provocative = [
        {"icon":"🌍","type":"SOVEREIGN THOUGHT","headline":"Trinidad runs on oil. What do you run on?","sub":"Every person needs a primary resource — a thing they produce that others need. What's yours?","color":"var(--gold)"},
        {"icon":"🔥","type":"FRICTION POINT","headline":"The version of you with no excuses. What would they do today?","sub":"Not the ideal version. Just the honest one. The one who stops explaining why not.","color":"var(--gold)"},
        {"icon":"📡","type":"INTELLIGENCE SIGNAL","headline":"Information asymmetry is the only real edge left.","sub":"The person who knows the price before the price changes wins. SOVEREIGN exists for that reason.","color":"#5b8fc9"},
        {"icon":"⚡","type":"PATTERN INTERRUPT","headline":"Every goal you have right now — how many have a specific date attached?","sub":"Goals without dates are just preferences. Check yours.","color":"var(--gold)"},
        {"icon":"🧠","type":"COGNITIVE EDGE","headline":"Your best thinking happens before 11am or after 9pm.","sub":"Every expert says it. Most people schedule their hardest work at the worst time.","color":"#a07ee0"},
        {"icon":"💡","type":"MARKET INTELLIGENCE","headline":"The Caribbean market rewards the person who shows up consistently.","sub":"Not the most talented. Not the most funded. The most consistent. Are you showing up?","color":"var(--gold)"},
        {"icon":"🎯","type":"CLARITY SIGNAL","headline":"What is the single thing, if done today, that makes everything else easier?","sub":"Gary Keller called it the One Thing. SOVEREIGN calls it your primary node.","color":"#4aad74"},
        {"icon":"🔮","type":"DATA TRUTH","headline":"Your mood logs, goal progress, and hydration tell a story you haven't read yet.","sub":"Open your VECTOR OS dashboard and look at the last 30 days honestly.","color":"var(--gold)"},
    ]

    # Try live news headline
    try:
        news_items = get_news("https://www.theguardian.com/world/rss", 8)
        if news_items:
            item = random.choice(news_items[:6])
            signals.append({
                "icon":"📰","type":"LIVE SIGNAL",
                "headline": item["title"],
                "sub": "Live from The Guardian — happening right now in the world.",
                "color":"var(--dim)",
                "link": item.get("link","#")
            })
    except: pass

    # Pick — prefer data signals 60% of the time if available
    data_signals = [s for s in signals if s["type"] not in ["LIVE SIGNAL"]]
    if data_signals and random.random() < 0.65:
        return random.choice(data_signals)
    elif signals:
        return random.choice(signals)
    else:
        return random.choice(provocative)

# Generate signal once per session load
if "daily_signal" not in st.session_state:
    st.session_state.daily_signal = generate_signal()
signal = st.session_state.daily_signal
...
<div class="vos-header">
    <div>
        <div class="vos-wordmark">SOVEREIGN</div>
        <div class="vos-sub">Transparency · CRM · AI · TT Market · Personal OS</div>
    </div>
    <div class="vos-time">
        <strong>{greet}</strong><br>
        {now.strftime("%A, %B %d, %Y").upper()}<br>
        {now.strftime("%H:%M")} AST &nbsp;·&nbsp; {dmsg}
    </div>
</div>
...
<div class="signal-card">
    <div class="signal-badge">◈ {signal["type"]} &nbsp;·&nbsp; {now.strftime("%A %d %b").upper()}</div>
    <div class="signal-headline">{signal["icon"]} &nbsp;{signal["headline"]}</div>
    <div class="signal-sub">{signal["sub"]}</div>
    <div class="signal-tag">Signal refreshes on next load &nbsp;·&nbsp; tap to dismiss</div>
</div>
{sig_link_close}
""", unsafe_allow_html=True)

col_dismiss, _ = st.columns([1,4])
if col_dismiss.button("↺ New Signal", key="new_signal"):
    st.session_state.daily_signal = generate_signal()
    st.rerun()
